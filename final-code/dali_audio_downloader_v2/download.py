"""v2 YouTube → native-container audio downloader for the DALI English subset.

Differences vs v1:
* Keeps yt-dlp's native bestaudio (m4a / webm) — no MP3 postprocess.
* 8 worker threads + concurrent_fragment_downloads=4 for higher parallelism.
* Validates with torchaudio.info (handles m4a/webm; soundfile does not).
* Reuses v1's hard-won learnings: cookies-from-browser + remote_components=ejs:github.
"""
...
COOKIES_FROM_BROWSER = os.environ.get(
    "DALI_COOKIES_FROM_BROWSER", "chrome"
).strip() or None
...
log = logging.getLogger("dali_dl_v2")
...
def yt_dl_opts(out_path_stem: Path) -> dict:
    opts: dict = {
        ...
        "prefer_ffmpeg": True,
        "nopart": False,
        # No postprocessor — keep native container.
...
def find_existing(dali_id: str) -> Path | None:
    """Return any existing audio file for this dali_id, or None."""
...
def validate_audio(path: Path) -> tuple[bool, float]:
    """Return (ok, duration_seconds) using ffprobe (handles m4a / webm / opus)."""
    ...
    try:
        ...
        return False, 0.0
    if out.returncode != 0:
        log.warning("validate-fail %s :: rc=%d %s", path.name, out.returncode, out.stderr[:80])
        return False, 0.0
    try:
...
def download_one(candidate: dict, state: State) -> dict:
...
def main() -> int:
    ...
    candidates = candidates[:ATTEMPT_TARGET]

    print(f"Loaded {len(candidates)} candidates")
    print(
        f"Stop when: success>={SUCCESS_TARGET} or attempts>={ATTEMPT_TARGET} "
    ...
    log.info(
        "RUN START candidates=%d attempt_target=%d success_target=%d budget=%ds workers=%d",
        len(candidates),
        ATTEMPT_TARGET,
        SUCCESS_TARGET,
        TIME_BUDGET_SECONDS,
    ...
    def should_stop() -> bool:
        ...
        return (
            state.successful >= SUCCESS_TARGET
    ...
    def record(result: dict) -> None:
        ...
        with state.lock:
            ...
            if now - last_progress_line >= PROGRESS_INTERVAL:
                ...
                print(
                    f"Elapsed {fmt_hms(elapsed)} — attempted {state.attempted} — "
    ...
    with ThreadPoolExecutor(max_workers=N_WORKERS) as pool:
        ...
        def fill():
            while len(in_flight) < N_WORKERS * 2 and pending and not should_stop():
                cand = pending.pop(0)
                fut = pool.submit(download_one, cand, state)
                in_flight[fut] = cand
    ...
    print()
    print("V2 AUDIO DOWNLOAD COMPLETE")
    print(f"Attempted:   {state.attempted}")
    print(f"Successful:  {state.successful} (of which {state.skipped} were skip-existing)")
    print(f"Failed:      {state.failed}")
    print(f"Manifest:    {MANIFEST_PATH.resolve()}")
    print(f"Audio dir:   {AUDIO_DIR.resolve()}")
    print(f"Elapsed:     {fmt_hms(elapsed)}")
    log.info(
        "RUN END attempted=%d success=%d failed=%d elapsed=%.1fs",
        state.attempted,
        state.successful,
        state.failed,
        elapsed,
